...
public class ModManager {
    ...
    public FetchTreeResult fetchTree(String name, Version version, String comparison){
        ...
        Map<String, Tree<ModRelease>> releaseTrees = new ConcurrentHashMap<>();
        Tree<ModRelease> result = new Tree<>((Tree<ModRelease>) null);
        CopyOnWriteArrayList<Dependency> missing = new CopyOnWriteArrayList<>();
        Map<Tree<ModRelease>, List<String>> laterBranch = new ConcurrentHashMap<>();
        ...
        for(Map.Entry<Tree<ModRelease>, List<String>> later : laterBranch.entrySet()){
            for(String dep : later.getValue()){
                Tree<ModRelease> branch = releaseTrees.get(dep);
                if(branch != null){
        ...
        Tree<ModRelease> modRoot = result.getBranches().iterator().next();
    ...
    protected class FetchMod implements Runnable {
        ...
        private final Map<String, Tree<ModRelease>> releaseTrees;
        private final Tree<ModRelease> root;
        private final CopyOnWriteArrayList<Dependency> missing;
        private final Map<Tree<ModRelease>, List<String>> laterBranch;
        ...
        public void run(){
            ...
            Tree<ModRelease> newRoot = root.addBranch(release);
            ...
            for(Dependency dependency : release.getDependencies()){
                ...
                if(!seen.containsKey(dependency.getName()) && !resolving.contains(dependency.getName())){
                    ...
                    Tree<ModRelease> modReleaseTree = releaseTrees.get(dependency.getName());
    ...
    public class FetchTreeResult {
        ...
        private final Tree<ModRelease> result;
